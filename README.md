...
| **🛡️ Security Scanner** | 12+ built-in rules check for secrets, dangerous commands, and malware. SARIF output for GitHub Code Scanning. |
...
```bash
brew tap yeasy/tap
brew install --cask yeasy/tap/ask          # CLI version
...
| `ask gui` | Launch the native desktop UI (desktop builds only; use `ask serve` otherwise) |
